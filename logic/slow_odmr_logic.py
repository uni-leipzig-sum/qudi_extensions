...
class SlowODMRLogic(GenericLogic):
    ...
    def _scan_odmr_line(self):
        ...
        with self.threadlock:
            ...
            # Acquire count data
            new_counts = np.zeros(self.odmr_plot_x.shape[0])
            for i in range(self.odmr_plot_x.size):
                self._mw_device.trigger()
                new_counts[i] = self._odmr_counter.count_odmr(length=1)[0]
