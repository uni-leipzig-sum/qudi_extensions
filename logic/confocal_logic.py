    ...
    def _scan_line(self):
        ...
        if self.stopRequested:
            with self.threadlock:
                ...
                # make a line from the current position to the cursor position
                rs = self.return_slowness
                lsx = np.linspace(image[self._scan_counter-1, 0, 0], self._current_x, rs)
                lsy = np.linspace(image[self._scan_counter-1, 0, 1], self._current_y, rs)
                lsz = np.linspace(image[self._scan_counter-1, 0, 2], self._current_z, rs)
                if n_ch <= 3:
                    stop_line = np.vstack([lsx, lsy, lsz][0:n_ch])
                else:
                    stop_line = np.vstack(
                        [lsx, lsy, lsz, np.ones(lsx.shape) * self._current_a])
                # move to the cursor position, counts are thrown away
                self._scanning_device.scan_line(stop_line)
                if self._zscan:
        ...
        # continues scanning
        if self.continued:
            # line from cursor position to beginning of the line
            rs = self.return_slowness
            ...
            if n_ch <= 3:
                cont_line = np.vstack([lsx, lsy, lsz][0:n_ch])
            else:
                cont_line = np.vstack(
                    [lsx, lsy, lsz, np.ones(lsx.shape) * self._current_a])
            # move to the start position of the scan, counts are thrown away
            self._scanning_device.scan_line(cont_line)
            self.continued = False
            ...
            if self._scan_counter >= np.size(self._image_vert_axis):
                ...
                if n_ch <= 3:
                    ...
                    end_line = np.vstack(
                        [lsx, lsy, lsz, np.ones(lsx.shape) * self._current_a])
                # move to the cursor position, counts are thrown away
                self._scanning_device.scan_line(end_line)
